...
def load_manifest(source_root: Path, validate_sources: bool = True) -> PackManifest:
    ...
    if "migration" in raw:
        raise ValueError("manifest.migration is no longer supported")
    ...
    for index, raw_asset in enumerate(raw_assets):
        ...
        if "previous_hashes" in raw_asset:
            raise ValueError(f"{context}.previous_hashes is no longer supported")
...
def synchronize(
    ...
    for asset in scaffold_assets:
        upgrade_action = upgrade_scaffold_asset(
            source_root,
            target_root,
            asset,
            dry_run,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Synchronize a managed profile and optionally scaffold project files."
    )
    parser.add_argument(
        "--source",
